...
const AuthorCard = ({ name, image }) => (
  <div className="text-center">
    <div
      className={`mx-auto mb-2 border rounded-circle d-flex align-items-center justify-content-center bg-light ${styles.iconCircle}`}
    >
      <img
        src={image}
        alt={name}
        className={`${styles.iconImage} img-fluid`}
      />
    </div>
...
const LogCard = ({ image }) => (
  ...
  >
    <img src={image} alt="Log" className={`${styles.logImage} img-fluid`} />
  </div>
);
